...
int main(int argc , char *argv[])
{
	...
	recv(new_socket, &e_recv , sizeof(e_recv), 0); // get public thing , generator and prime
	send(new_socket , &e, sizeof(e), 0); 

	printf("Received client's public key: %ld\n", e_recv);
	...
	rsa_decrypt(gen, n, e_recv);
	rsa_decrypt(gen, n, d);
	rsa_decrypt(prime, n, e_recv);
	rsa_decrypt(prime, n, d);
	rsa_decrypt(client_public_key, n, e_recv);
	rsa_decrypt(client_public_key, n, d);
	...
	rsa_encrypt(shared_key, n, e_recv);
	rsa_encrypt(shared_key, n, d);
