...
int main(int argc , char *argv[])
{
	...
	long int n, d, e, e_recv;
	gen_private_public_pair(&e, &d, &n);
...
// *********** This is the line you need to edit ****************
	...
	send(socket_desc , &e, sizeof(e), 0); 
	recv(socket_desc, &e_recv , sizeof(e_recv), 0); // get public thing , generator and prime

	printf("Received server's public key: %ld\n", e_recv);
	printf("Perfomring signed diffie hellman...\n");

	rsa_encrypt(generator, n, e_recv);
	rsa_encrypt(generator, n, d);
	rsa_encrypt(prime, n, e_recv);
	rsa_encrypt(prime, n, d);
	rsa_encrypt(client_public_key, n, e_recv);
	rsa_encrypt(client_public_key, n, d);
	...
	rsa_decrypt(server_public_key, n, e_recv);
	rsa_decrypt(server_public_key, n, d);


	//Calculate a shared key for future encryptions
	shared_key = calc_key(server_public_key, priv_key, prime);
	printf("Session shared key: %d\n", shared_key);
	...
	printf("What do you want to send to the server. (CTRL + C to exit)\n");
